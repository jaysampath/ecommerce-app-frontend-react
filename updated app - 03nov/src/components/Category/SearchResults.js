...
const SearchResults = (props) => {
  ...
  useEffect(() => {
    const fetchSearchItems = async () => {
      ...
      const response = await fetch(
        `http://localhost:8080/item/search/${searchText}`
      );
      if (!response.ok) {
  ...
  if(!isFetched){
    return (
      <div className={classes.loaderDiv}>
        ...
        <Loader type = "Bars" color="white" height={100} width={110} />
      </div>
    );
  }
  ...
  return (
    <div className={classes.searchResultsDiv}>
      {isFetched && (
        <div>
          <h1 style={{color:"yellow",textAlign:"center"}}>showing results for "{searchText}"</h1>
