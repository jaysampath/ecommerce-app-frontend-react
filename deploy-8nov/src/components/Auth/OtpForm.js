...
const OtpForm = (props) => {
  ...
  useEffect(() => {
    const generateOtp = async () => {
      ...
      const response = await fetch(
        `http://localhost:8080/otp/generate/${email}`
      );
      if (!response.ok) {
        throw new Error(
       ...
       // console.log("signup response", data);
        if (data["status"] !== 202) {
          ...
          const locationTimer = setTimeout(() => {
            authCtx.login(email);
            window.location.href = "https://ecommerce-app-frontend-react.vercel.app";
  ...
  const otpSubmitHandler = (event) => {
    ...
    fetch(`http://localhost:8080/otp/validate/${email}/${enteredOtp}`)
      ...
      .then((data) => {
       // console.log(data);
        ...
        if (data.status === 202) {
          setError(null);
          setSuccess("otp is valid, setting up your account...");
          registerNewUser();
        }
      })
      .catch((error) => {
      ...
      <div className={classes.otpFormDiv}>
        ...
        <form onSubmit={otpSubmitHandler}>
          ...
          <div className={classes.resendDiv}>
            Haven't received yet?  <button className={classes.resend} onClick={refreshPageHandler}>Resend Otp</button>
          </div>
          <div className={classes.actions}>
            <button type="submit" className={classes.submit}> Submit </button>
            <button type="button" onClick={cancelButtonHandler} className={classes.cancel}>
              Cancel
            </button>
          </div>
        </form>
