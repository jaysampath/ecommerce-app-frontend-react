  ...
  return async (dispatch) => {
    const fetchCart = async () => {
      const response = await fetch(
        `http://localhost:8080/cart/${loggedInUser}`
      );
...
export const addItemToCart = (itemId, itemQuantity, userEmail) => {
  ...
  return async (dispatch) => {
      ...
      const addItem = async() => {
        ...
        const response = await fetch(`http://localhost:8080/cart/add/${userEmail}`,{
          method:"POST",
          body:JSON.stringify({
            "itemId":itemId,
...
export const addItemQuantityInCart = (loggedInUser,itemId) => {
  ...
  return async (dispatch) => {
    ...
    const addItemQuantity = async() => {
       const response = await fetch(`http://localhost:8080/cart/change/add/${loggedInUser}/${itemId}`,{
           method:"POST"
       });
...
export const reduceItemQuantityInCart = (loggedInUser,itemId) => {
  ...
  return async (dispatch) => {
    ...
    const reduceItemQuantity = async() => {
       const response = await fetch(`http://localhost:8080/cart/change/reduce/${loggedInUser}/${itemId}`,{
           method:"POST"
       });
...
export const deleteItemFromCart = (loggedInUser,itemId) => {
  ...
  return async (dispatch) => {
    ...
    const deleteItem = async() => {
       const response = await fetch(`http://localhost:8080/cart/delete/${loggedInUser}/${itemId}`,{
           method:"POST"
       });
